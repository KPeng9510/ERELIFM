...
if __name__ == '__main__':
    ...
    if dataset == 'PACS':
        train_dir = '/hkfs/work/workspace/scratch/fy2374-got/workspace/ijcai_folders/Neurips/Homework3-PACS/PACS_train'
        val_dir = '/hkfs/work/workspace/scratch/fy2374-got/workspace/ijcai_folders/Neurips/Homework3-PACS/PACS_crossval'
        test_dir = ['/hkfs/work/workspace/scratch/fy2374-got/workspace/ijcai_folders/Neurips/Homework3-PACS/PACS_train', '/hkfs/work/workspace/scratch/fy2374-got/workspace/ijcai_folders/Neurips/Homework3-PACS/PACS_crossval']
        sub_batch_size = batch_size // 2
        small_img = False
    elif dataset == 'OfficeHome':
        train_dir = '/pfs/work7/workspace/scratch/fy2374-workforuc2/NIPS/Homework3-PACS/OfficeHomeDataset_10072016/_train'
        val_dir = '/pfs/work7/workspace/scratch/fy2374-workforuc2/NIPS/Homework3-PACS/OfficeHomeDataset_10072016/_crossval'
        test_dir = '/pfs/work7/workspace/scratch/fy2374-workforuc2/NIPS/Homework3-PACS/OfficeHomeDataset_10072016/_test'
    ...
    loader = MultiDomainData(root_dir=train_dir, domain=source_domain, classes=known_classes, nsr=nsr, asymetric=asym,transform=get_transform("train", small_img=small_img),transform2=get_transform("train", small_img=small_img), simclr_transform=transform, train=True)
    N = len(loader)
    ...
    if net_name == 'resnet18':
        ...
        net = muticlassifier(net=net_, num_classes=num_classes, feature_dim=768)
    ...
    for epoch in range(epochs):
        ...
        all_gradients = []
        if epoch%3 ==0:
            for g in optimizer_simclr.param_groups:
                g['lr'] = 0.001
        if epoch%3 == 1:
            for g in optimizer_simclr.param_groups:
                g['lr'] = 0.001
        ...
        for weight in net.parameters():
            weight.fast = None
        net.zero_grad()
        for batch_idx, (img, img2, labels_x, labels_original, target, index, path) in enumerate(loader):
            net.train()
            rep = net_res_clr.backbone(img.cuda())
            all_rep.append(rep.detach().cpu().numpy())
            all_clean_labels.append(labels_original.detach().cpu().numpy())
            all_noisy_labels.append(labels_x.detach().cpu().numpy())
            all_domain.append(target.detach().cpu().numpy())
            z_logits = net(img.cuda())

            edlloss = edl_soft_mse_loss(z_logits, labels_x.cuda())
            celoss = criterion(z_logits, labels_x.cuda())

            all_loss.append(celoss.detach().cpu().numpy())
            all_loss_edl.append(edlloss.detach().cpu().numpy())
            all_index.append(index.detach().cpu().numpy())

            loss_correct =  edlloss.mean()
            ...
            for loss in edlloss:
                ...
                grad.append(gradients[-3].detach().cpu())
                ...
                if test_u != None:
                    ...
                    with torch.no_grad():
                        for input, *_ in val_u:
                            ...
                            #output =  head_simclr(output, net_res)
                            output = net(input)
                            output = F.softmax(output, 1)
                            b_output = output#net.b_forward(x=input)
        ...
        #scheduler.step()
        all_rep = np.concatenate(all_rep)
        ...
        all_loss_edl = np.concatenate(all_loss_edl) 
        all_index = np.concatenate(all_index)
        all_gradients = np.concatenate(all_gradients)
        #all_loss_simclr = np.concatenate(all_loss_simclr)
        total_rep.append(all_rep)
        total_clean_labels.append(all_clean_labels)
        total_noisy_labels.append(all_noisy_labels)
        total_loss.append(all_loss)
        total_loss_edl.append(all_loss_edl)
        total_index.append(all_index)
        total_domain.append(all_domain)
        total_gradients.append(all_gradients)
        #total_loss_simclr.append(all_loss_simclr)'''
    import pickle as pkl
    f = open('data_PACS_'+target_domain[0]+'_0_5_a_re.pkl', 'wb')
    pkl.dump(file=f, obj=[total_rep, total_clean_labels, total_noisy_labels, total_loss, total_loss_edl, total_index, total_domain, total_gradients])
    f.close()