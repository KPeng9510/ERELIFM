...
def get_transform(instr, small_img=False, color_jitter=False, random_grayscale=True):
    if small_img == False:
        ...
        img_tr2 = [transforms.RandomResizedCrop((256, 256), (0.8, 1.0))]
        img_tr2.append(transforms.RandomHorizontalFlip(0.5))
        if color_jitter:
            img_tr2.append(transforms.ColorJitter(brightness=0.4, contrast=0.4, saturation=0.4, hue=0.4))
        if random_grayscale:
            img_tr2.append(transforms.RandomGrayscale(0.1))
        img_tr2.append(transforms.ToTensor())
        img_tr2.append(transforms.Normalize([0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]))

        #img_tr.append(transforms.GaussianBlur(3, sigma=(0.1, 2.0)))
        #img_tr.append(transforms.transforms.AugMix())
        transform_map = {     
            "train": transforms.Compose(img_tr),
            "train2": transforms.Compose(img_tr2),
        ...
        if color_jitter:
            img_tr.append(transforms.ColorJitter(brightness=0.4, contrast=0.4, saturation=0.4, hue=0.4))
        if random_grayscale:
            img_tr.append(transforms.RandomGrayscale(0.5))
        #img_tr.append(transforms.GaussianBlur(3, sigma=(0.1, 2.0)))
        #img_tr.append(transforms.RandomResizedCrop((32,32), scale=(0.8, 1.0), ratio=(0.5, 2.0)))
        #img_tr.append(transforms.RandomRotation(20))
        img_tr.append(transforms.ToTensor())
