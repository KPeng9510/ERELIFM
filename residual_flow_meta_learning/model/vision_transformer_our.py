...
@register_model
def vit_base_patch16_224_k(pretrained=False, **kwargs):
    ...
    if pretrained:
        load_pretrained(
            model, num_classes=model.num_classes, pretrained_cfg=default_cfgs['vit_base_patch16_224'], in_chans=kwargs.get('in_chans', 3), filter_fn=_conv_filter)
    return model
...
class ViT_CLIP(nn.Module):
    ...
    def forward(self, img):
        return self.proj(self.model.encode_image(img))
@register_model
def vit_clip(pretrained=False, **kwargs):
    model=ViT_CLIP()
    #model = VisionTransformer(
    #    patch_size=16, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
    #    norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
