...
class MutiClassifier(nn.Module):
    def __init__(self, net, num_classes, feature_dim=512):
        ...
        #self.classifier = nn.Sequential(nn.Linear(feature_dim, int(feature_dim/2)), nn.ReLU(), nn.Dropout(0.5), nn.Linear(int(feature_dim/2), num_classes))#Linear_fw(feature_dim, self.num_classes)
...
class MutiClassifier_(nn.Module):
    def __init__(self, net, num_classes, feature_dim=512):
        ...
        self.net = net
        self.num_classes = num_classes
        #self.b_classifier = Linear_fw(feature_dim, self.num_classes)
        self.b_classifier = Linear_fw(feature_dim, self.num_classes)
...
class MutiClassifier_simclr(nn.Module):
    def __init__(self, num_classes=6, feature_dim=512):
        super(MutiClassifier_simclr, self).__init__()
        #self.net = net
        self.num_classes = num_classes
        self.classifier = Linear_fw(feature_dim, self.num_classes)
...
def resnet152(progress=True, num_classes=None):
    model = ResNetFast(Bottleneck, [3, 8, 36, 3])
