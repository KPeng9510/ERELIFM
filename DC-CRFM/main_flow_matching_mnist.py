...
model = 'convnet'
target = ['mnist']#['']
source = ['mnist','syn', 'mnist_m', 'svhn']#['sketch', 'cartoon', 'art_painting', 'photo']
asym = False
nsr = 0.5
gpu = 0
...
if __name__ == '__main__':
    ...
    if asym == True:
        parser.add_argument('--save-dir', default='path/to/Digits_'+model+'_asym_snr_'+ns+'_'+target[0]+'/')
    else:
        parser.add_argument('--save-dir', default='path/to/Digits_'+model+'_sym_snr_'+ns+'_'+target[0]+'/')
    ...
    if dataset == 'DigitsDG':
        train_dir = 'path/to/DDG_train'
        val_dir = 'path/to//DDG_crossval'
        test_dir = ['path/to//DDG_train', 'path/to//DDG_crossval']
    ...
    #clean_indexes = [torch.Tensor(item).long() for item in clean_indexes]
    #print(clean_indexes)
